...
int s21_divide_integer(s21_decimal dividend, s21_decimal divisor, s21_decimal *quotient, s21_decimal *remainder) {
  ...
  // Классический алгоритм длинного деления: рассматриваем все 96 бит dividend с 95-го до 0-го.
  // В процессе накапливаем остаток (remainder) и выставляем соответствующие биты в quotient.
  for (int i = 95; i >= 0; i--) {
    ...
    // Если накопленный remainder >= divisor, вычитаем divisor и устанавливаем бит i в quotient
...
int s21_add_core(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  return carry ? AR_OVERFLOW : AR_OK;
}
