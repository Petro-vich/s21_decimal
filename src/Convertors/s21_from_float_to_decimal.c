...
#include <stddef.h>  // Добавлено для определения NULL
#include <math.h>
#include <stdint.h>
...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
    ...
    // Определение знака
    if (src < 0) {
        dst->bits[3] |= 0x80000000;  // делаем 31-й бит = 1 с помощью маски и побит. оператора(ИЛИ)
        src = -src;  // если оно было отрицательным, делаем его положительным для дальнейшего преобразования
    }
    ...
    if (int_part > 0xFFFFFFFF) {
        ...
        dst->bits[1] = 0;  // т.к. самое большое 7-знаковое число = 27 бит, второй байт не юзается
    }
